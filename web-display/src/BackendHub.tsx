...
function HubVenueSettings({ venue, busy, onSave, onUploadLogo, onUseDefaultLogo, onClose }: {
  ...
  return <section className="hub-venue-settings">
    <div className="hub-card-title"><h3>Оформление: {venue.name}</h3><button type="button" onClick={onClose}>Закрыть</button></div>
...
export function BackendHub() {
  ...
  const filteredVenues = overview?.venues.filter(venue => `${venue.name} ${venue.code}`.toLocaleLowerCase().includes(venueSearch.trim().toLocaleLowerCase())) ?? [];
  const copyDisplayUrl = async () => {
    if (!createdDisplayUrl) return;
    try { await navigator.clipboard.writeText(createdDisplayUrl); setCopyNotice("Ссылка скопирована"); window.setTimeout(() => setCopyNotice(""), 2500); }
    catch { setCopyNotice("Не удалось скопировать ссылку"); }
  };

  if (!hubToken) return <main className="hub-login">
    <div className="hub-login-card"><span className="hub-kicker">InteractiveFoodMenu</span><h1>Backend Hub</h1><p>Единый центр управления серверной частью.</p><input type="password" autoComplete="current-password" placeholder="Ключ оператора" value={accessKey} onChange={event => setAccessKey(event.target.value)} onKeyDown={event => { if (event.key === "Enter") void login(); }} /><button type="button" disabled={busy || !accessKey} onClick={() => void login()}>{busy ? "Проверяем…" : "Войти в хаб"}</button>{error && <p className="hub-error" role="alert">{error}</p>}</div>
  </main>;
  ...
  return <main className="backend-hub">
    ...
    <section className="hub-content">
      ...
      {overview && <>
        ...
        {tab === "venues" && <><div className="hub-venue-toolbar"><label><span className="sr-only">Поиск точек</span><input type="search" placeholder="Найти точку или код…" value={venueSearch} onChange={event => setVenueSearch(event.target.value)} /></label><small>{filteredVenues.length} из {overview.totals.venues} точек</small></div><div className="hub-displays-overview">{filteredVenues.map(venue => <HubDisplays key={`displays-${venue.id}`} token={hubToken} venue={venue} />)}</div><div className="hub-grid">
          <section className="hub-card"><div className="hub-card-title"><h3>Все точки</h3><small>{filteredVenues.length} из {overview.totals.venues}</small></div><div className="hub-table-wrap"><table><thead><tr><th>Точка</th><th>Код</th><th>Версии</th><th>Обновлена</th><th></th></tr></thead><tbody>{filteredVenues.map(venue => <tr key={venue.id}><td><b>{venue.name}</b><small>{venue.id}</small></td><td><code>{venue.code || "—"}</code></td><td><small>staff {venue.staffVersion} · display {venue.displayVersion}</small></td><td>{dateTime(venue.updatedAt)}</td><td className="hub-actions"><button type="button" onClick={() => void openDisplay(venue.id)}>Экран меню</button><a className="hub-action-link" href={`${window.location.origin}/staff?venue=${encodeURIComponent(venue.code)}`} target="_blank" rel="noreferrer">Staff</a><button type="button" onClick={() => setEditingVenueId(venue.id)}>Оформление</button><button type="button" onClick={() => void rotatePin(venue.id, venue.code)}>Сменить PIN</button><button type="button" className="danger" onClick={() => void revokeSessions(venue.id)}>Отозвать</button><button type="button" className="danger" onClick={() => void deleteVenue(venue.id, venue.name)}>Удалить</button></td></tr>)}</tbody></table></div>{editingVenue && <HubVenueSettings venue={editingVenue} busy={busy} onClose={() => setEditingVenueId("")} onSave={settings => saveVenueSettings(editingVenue.id, settings)} onUploadLogo={file => uploadVenueLogo(editingVenue.id, file)} onUseDefaultLogo={() => useDefaultLogo(editingVenue.id)} />}</section>
          <section className="hub-card hub-create"><h3>Новая точка</h3><label>Название<input placeholder="Кофейня на Невском" value={newVenue.name} onChange={event => setNewVenue({ ...newVenue, name: event.target.value })} /></label><label>Код для входа<input placeholder="nevsky" pattern="[a-z0-9-]{3,32}" value={newVenue.venueCode} onChange={event => setNewVenue({ ...newVenue, venueCode: event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "") })} /><small>3–32 латинских символа, цифры или дефис.</small></label><label>PIN<input type="password" inputMode="numeric" maxLength={6} placeholder="6 цифр" value={newVenue.pin} onChange={event => setNewVenue({ ...newVenue, pin: event.target.value.replace(/\D/g, "") })} /></label><button type="button" disabled={busy || !newVenue.name || newVenue.venueCode.length < 3 || newVenue.pin.length !== 6} onClick={() => void createVenue()}>Создать точку</button>{createdDisplayUrl && <div className="hub-secret"><b>Ссылка первого экрана</b><p>Она показывается только сейчас. Сохраните её безопасно.</p><textarea readOnly value={createdDisplayUrl} /><button type="button" onClick={() => void copyDisplayUrl()}>{copyNotice || "Скопировать"}</button></div>}</section>
        </div></>}
      ...
      {copyNotice && <p className="hub-banner hub-success" role="status">{copyNotice}</p>}
    </section>
  </main>;
}
