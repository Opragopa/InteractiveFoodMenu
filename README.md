...
Прототип состоит из Android-приложения сотрудника, веб-кабинета сотрудника, web-экрана для телевизора и Firebase backend. Сотрудник управляет позициями, отмечает их флажком «Нет в наличии» и может добавлять меню из CSV. Открытый экран получает изменения через Firestore и зачёркивает недоступные блюда.
...
Узнайте LAN-IP Mac (`ifconfig`, например `192.168.0.183`). Телефон и Mac должны быть в одной сети, а macOS Firewall должен разрешать входящие подключения Node.
...
Экран собирается также в ES5-варианте с полифилами для webOS 1–4 (WebKit и Chromium 38/53). На таких ТВ не используются reCAPTCHA Enterprise и IndexedDB-постоянство Firebase: ссылка экрана защищена одноразовым секретом и Firebase Auth. После изменения фронтенда перезапустите dev-сервер либо разверните новую сборку Hosting; телевизор не получает обновление исходного кода сам.
...
После пересборки и деплоя ссылки экрана будут вести на `/display/<token>.<secret>`. Телевизор получает готовый HTML без React, Firebase SDK и JavaScript; меню обновляется автоматически каждые 20 секунд. Для локального запуска перезапустите Functions Emulator и web dev server.
